...
research energy actually is, and it is in rhythm, timing and the body.
...
- **The browser run counted two sources twice.** `10.1080/09298215.2013.818038`
  ...
  `mtilt.bib` and `works-pitch-ontology.bib`, and `tools/citations.html` did not
  deduplicate (its list is now generated by `tools/build-citations-dois.py`). So the header's 64 sources are **62**, and 21 of the 836 rows are
